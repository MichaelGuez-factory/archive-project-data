function archiveProjects_v2(scoroIdsToArchive) {
  ...
  if (scoroIdsToArchive.length > 0) {
    ...
    const sourceSheetTabName = `Project Export`;
    const spreadsheetTotalColumnCount = 19; //Only pull data up to column S (19).  
    ...
    collectedData.forEach((record) => {
      ...
      if (scoroIdsToArchive.includes(ScoroReference)) {
        ...
        // --- MODIFICATION START ---
        // Set the 4th column (Index 3) to "Completed"
        record[3] = "completed";
        // --- MODIFICATION END ---
...
function removeProjects_v2(scoroIdsToArchive) {
